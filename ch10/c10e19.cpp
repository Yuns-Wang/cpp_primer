...
void stable_biggies(vector<string> &words, const int length)
{
    elimDups(words);
    auto second_part_start = std::stable_partition(words.begin(), words.end(), [length](const string &s){return s.size() < length;});
    cout << "长度大于" << length << "的单词共有" << words.end() - second_part_start << "个。分别为：" << endl;
    std::for_each(second_part_start, words.end(), [](const string &s){cout << s << ", ";});
    cout << endl;
}
...
int main()
{
    ...
    // 用biggies_stable()函数实现功能
    cout << "==============stable_bigges()==================" << endl;
    stable_biggies(words, length);
    return 0;
}
